...
람다(lambda) 함수
    ...
    작성할 수 있고, 재사용되지 않는 1회성 함수를 만들때 사용한다.
'''
...
'''
map()함수
    : 전달된 데이터를 동일 함수에 반복적으로 적용시켜주는 역할을 한다.
    for문과 같은 반복문을 사용하지 않아도 지정한 함수로 인수를 여러번 전달해서
    그 결과를 List 형태로 반환하는 유용한 함수이다.
    형식]
        map(람다식, 파라미터);
'''
print('### 람다식과 map함수1 ###')
...
형식]
    ...
    - 람다식에서 조건부 표현식을 사용할때는 :를 붙이지 않는다.
    - if를 사용했다면 반드시 else를 사용해야 한다.
    - elif는 사용할 수 없다. 따라서 2개 이상의 조건도 if를 연속적으로 사용해야 한다.
'''
print('### 람다식과 map함수2 ###')
...
result_list2 = list(map(multiLambda, list_data2))
...
'''
filter함수
    : 반복가능한 객체에서 특정 조건에 맞는 요소만 가져오는데 지정된 람다식에서
    true를 반환하는 것만 해당 요소를 반환한다. 형식은 map과 동일하다.
'''
print('### 람다식과 filter함수 ###')
powLambda = lambda y : y**2 # 인수를 제곱해서 반환하는 람다식 정의
list_data3 = [1,4,9,16,25,49,64,81,100]
result_list3 = list(map(powLambda, list_data3)) # map을 통해 리스트 원소의 제곱을 구한다.
print('result_list3', result_list3)

# 50 초과 & 1000 미만인 경우에만 true를 반환하는 람다식을 통해 원소를 필터링 한다.
...
'''
reduce함수
    : 반복 가능한 객체의 각 요소를 지정된 함수로 처리한 뒤 이전 결과에
    누적해서 반환하는 함수
    파이썬3부터는 내장 함수가 아니므로 functools 모듈을 import하여 사용해야 한다.
'''
print('### 람다식과 reduce함수 ###')
import functools, operator
'''
    1~10까지의 합을 반환한다. 람다식이 두 요소를 더해서 반환하도록 정의되었으므로
    처음엔 1과 2가 인수로 주어져 더해지고, 그 결과에 3을 더하는 구조를 가지게 된다.
'''
sum1 = functools.reduce(lambda i, j: i + j, range(1,11))
print("sum1=", sum1) # 결과: 55

# operator 모듈의 add() 함수를 사용하여 위와 동일한 결과값을 반환한다.
