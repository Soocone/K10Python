...
리스트(list)
    : 대괄호[] 안에 콤마로 항목을 구분하며, 대입연산자로 항목을 
    변경할 수 있는 시퀀스 자료형이다. 서로 다른 자료형의 항목으로도
...
# print('list2[5]:', list2[5]) # index out range 오류 발생
...
list1.insert(1, 99) # 지정된 인덱스 요소에 삽입. 이후로는 항목이 뒤로 밀려나게 됨
print('insert=>', list1)

print(list1.pop()) # 마지막 항목 반환 및 삭제
print('pop()=>', list1)

print(list1.remove(99)) # 처음 나오는 항목 99를 삭제
print('remove=>', list1)

print(list1.reverse()) # 리스트를 뒤집을 때 사용
print('reverse=>', list1)

'''
List Comprehension
    : 대괄호 안에 for루프로 반복적인 표현식을 실행해서 리스트 요소들을 초기화 하는 방법
    형식]
