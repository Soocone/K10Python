...
def calc(val):
    ...
    try: # 예외발생이 예상되는 지점을 try로 묶는다.
        ...
        if val[0]==100:
            print(no_var) #선언되지 않은 변수를 출력하므로 예외 발생
        elif val[0]==55:
            result= val[0] / 0 # 숫자를 0으로 나누므로 예외 발생
            print("결과", result)
...
calc([10, 20]) # 인덱스 에러
print('실행3')
calc([100, 101, 102, 103]) # NameError 예외 발생
print('실행4')
calc([55, 56, 57]) # 0으로 나누므로 예외 발생
...
try:
    ...
    try:
        ...
        print("파일 객체를 닫습니다")
        fp.close()
except IOError: # 현재 경로에 파일이 없으므로 예외 발생
    print("파일 에러 발생")
...
def print_gugudan(end_num):
    try:
        # 예외를 발생시킬 조건에서 해당 클래스를 raise 한다.
        if end_num >9:
